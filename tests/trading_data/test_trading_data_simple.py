...
# 添加项目根目录到路径
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '../../crypto_trading_agents'))
...
def test_trading_data_service():
    ...
    try:
        ...
        for timeframe, timeframe_data in data.items():
            if timeframe_data:
                print(f"   - {timeframe} 数据范围: {timeframe_data[0].get('timestamp', 'N/A')} 到 {timeframe_data[-1].get('timestamp', 'N/A')}")
