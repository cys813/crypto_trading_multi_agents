...
def get_rss_news_sentiment():
    ...
    for name, url in rss_feeds:
        try:
            ...
            for item in items[:20]:  # 分析最新20篇文章
                ...
                # 检查是否与ETH相关
                if any(keyword in text_content.lower() for keyword in ['eth', 'ethereum']):
    ...
    print(f"  📊 分析了 {len(all_articles)} 篇ETH相关文章")
...
def get_social_media_sentiment():
    ...
    try:
        url = "https://min-api.cryptocompare.com/data/v2/news/?lang=EN&categories=ETH"
        headers = {'User-Agent': 'CryptoSentimentBot/1.0'}
        request = urllib.request.Request(url, headers=headers)
